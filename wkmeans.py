...
class WKMeans():
    ...
    def _cluster_points(self):
        ...
        for index, x in enumerate(self.X):
            ...
            C[bestmukey].append(self.counts_per_data_point[index])
    ...
    def _reevaluate_centers(self):
        ...
        new_mu = []
        for k,c in zip(self.clusters, self.C):
            # For each key, add a new centroid (mu) by calculating the cluster
            # mean.
            k,c = np.array(k), np.array(c)
            new_mu.append(np.mean(np.multiply(k,np.expand_dims(c,0).repeat(k.shape[1],axis=0).T), axis=0))

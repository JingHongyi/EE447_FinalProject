...
def random_ip():
    with open("proxy_ip.txt", 'r') as f:
        ip = random.choice(f.readlines()).rstrip('\n')
...
def captcha_process(url):
    ...
    while soup.find('head').text[:9].find('安全验证 - 知乎')!=-1:
        try:
            ...
            driver.find_element_by_xpath('//*[@id="root"]/div/div[2]/section/div/label/input').click()
            driver.find_element_by_xpath('//*[@id="root"]/div/div[2]/section/div/label/input').clear()
            driver.find_element_by_xpath('//*[@id="root"]/div/div[2]/section/div/label/input').send_keys(captcha)
...
if __name__ == '__main__':
    ...
    thread_running(5,followers)
    
    #保存在本地
    f=open('followers_questions.txt','w',encoding='utf8')
    f.writelines(questions)
    f.close()
    f=open('followers_articles.txt','w',encoding='utf8')
    f.writelines(articles)
    f.close()
    f=open('followers_zhuanlans.txt','w',encoding='utf8')
    f.writelines(zhuanlans)
    f.close()
    f=open('followers_topics.txt','w',encoding='utf8')
    f.writelines(topics)
    f.close()
